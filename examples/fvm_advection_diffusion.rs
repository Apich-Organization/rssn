...
fn main() {
    ...
    const VELOCITY: (f64, f64) = (0.5, 0.3); // Moving top-right
    const TOTAL_TIME: f64 = 1.0;

    const DT: f64 = 0.005;

    // 2. Initialize Mesh with Pollutant (Gaussian Blob)
    println!("Initializing {}x{} mesh...", WIDTH, HEIGHT);
    ...
    let steps_per_frame = (TOTAL_TIME / DT / num_frames as f64) as usize;
    ...
    for frame in 0..=num_frames {
        ...
        #[cfg(feature = "output")]
        {
            let disp_grid = mesh_to_ndarray(&mesh);

            let path = format!("fvm_advection_frame_{:02}.png", frame);

            let mut conf = PlotConfig::default();

            conf.caption = format!(
                "Pollutant Transport \
                 - T={:.2}",
                frame as f64 * DT * steps_per_frame as f64
            );
            ...
            if let Err(e) = plot_heatmap_2d(&disp_grid, &path, Some(conf)) {
                eprintln!(
                    "Failed to plot \
                     frame {}: {}",
                    frame, e
        ...
        let bc = |i, j, w, h| i == 0 || i == w - 1 || j == 0 || j == h - 1;

        let new_values = solve_advection_2d(&mut mesh, VELOCITY, DT, steps_per_frame, bc);
...
fn mesh_to_ndarray(mesh: &Mesh2D) -> Array2<f64> {
    let mut grid = Array2::zeros((mesh.height, mesh.width));
    ...
    for j in 0..mesh.height {
        for i in 0..mesh.width {
            ...
            grid[[mesh.height - 1 - j, i]] = mesh.cells[idx].value;
        }
    }
