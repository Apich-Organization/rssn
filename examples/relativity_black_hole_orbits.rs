...
use rssn::physics::physics_sim::geodesic_relativity::GeodesicParameters;
use rssn::physics::physics_sim::geodesic_relativity::run_geodesic_simulation;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let scenarios = vec![
        ("Stable Orbit", stable_orbit_params, &CYAN),
        ("Plunging Orbit", plunging_orbit_params, &RED),
        ("Photon Orbit", photon_orbit_params, &YELLOW),
    ];

    // 2. Setup plotting area
    let output_file = "black_hole_orbits.png";
    ...
    let horizon_radius = 2.0 * black_hole_mass;

    let horizon_points: Vec<(f64, f64)> = (0..=100)
        .map(|i| {
            let angle = i as f64 * 2.0 * std::f64::consts::PI / 100.0;

            (horizon_radius * angle.cos(), horizon_radius * angle.sin())
        })
        .collect();
    ...
    chart
        .draw_series(std::iter::once(Polygon::new(
            ...
            WHITE.mix(0.2).filled(),
        )))?
        .label("Event Horizon (2M)")
        .legend(|(x, y)| Circle::new((x, y), 5, WHITE.mix(0.2).filled()));
    ...
    for (name, params, color) in scenarios {
        println!("Simulating {}...", name);

        let path = run_geodesic_simulation(&params);

        chart
            .draw_series(LineSeries::new(path, color.stroke_width(2)))?
            .label(name)
            .legend(move |(x, y)| {
