...
fn main() {
    ...
    let initial_u = |x: usize, y: usize, z: usize| {
        let dx_cen = x as f64 - (WIDTH / 2) as f64;

        let dy_cen = y as f64 - (HEIGHT / 2) as f64;

        let dz_cen = z as f64 - (DEPTH / 2) as f64;

        let dist2 = dx_cen * dx_cen + dy_cen * dy_cen + dz_cen * dz_cen;
    ...
    let s_x = (C * config.dt / config.dx).powi(2);

    let s_y = (C * config.dt / config.dy).powi(2);

    let s_z = (C * config.dt / config.dz).powi(2);
    ...
    for step in 0..config.steps {
        ...
        for z in 1..DEPTH - 1 {
            for y in 1..HEIGHT - 1 {
                for x in 1..WIDTH - 1 {
                    let lap_x =
                        u_curr[(x + 1, y, z)] - 2.0 * u_curr[(x, y, z)] + u_curr[(x - 1, y, z)];

                    let lap_y =
                        u_curr[(x, y + 1, z)] - 2.0 * u_curr[(x, y, z)] + u_curr[(x, y - 1, z)];

                    let lap_z =
                        u_curr[(x, y, z + 1)] - 2.0 * u_curr[(x, y, z)] + u_curr[(x, y, z - 1)];

                    u_next[(x, y, z)] = 2.0 * u_curr[(x, y, z)] - u_prev[(x, y, z)]
                        + s_x * lap_x
                        + s_y * lap_y
                        + s_z * lap_z;
                }
        ...
        if step % 10 == 0 {
            #[cfg(feature = "output")]
            {
                ...
                if let Err(e) = plot_heatmap_2d(&array, &plot_path, Some(plot_config)) {
...
fn fdm_grid_3d_slice_to_array2(
    ...
    let (width, height, _) = match grid.dimensions() {
        | Dimensions::D3(w, h, d) => (*w, *h, *d),
        | _ => {
