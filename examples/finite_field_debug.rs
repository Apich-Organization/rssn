...
fn main() {
    ...
    let result = p1 + p2;
    println!("Addition result: degree={}, coeffs={:?}", result.degree(), result.coeffs.iter().map(|c| &c.value).collect::<Vec<_>>());
    ...
    println!("Division: quotient degree={}, remainder degree={}", quotient.degree(), remainder.degree());
    println!("Quotient coeffs: {:?}", quotient.coeffs.iter().map(|c| &c.value).collect::<Vec<_>>());
    println!("Remainder coeffs: {:?}", remainder.coeffs.iter().map(|c| &c.value).collect::<Vec<_>>());
}
