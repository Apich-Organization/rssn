...
fn main() {
    ...
    let initial_u = |x: usize, y: usize| {
        let dx1 = x as f64 - (WIDTH as f64 * 0.4);

        let dy1 = y as f64 - (HEIGHT as f64 * 0.4);

        let r1 = (-(dx1 * dx1 + dy1 * dy1) / 40.0).exp();

        let dx2 = x as f64 - (WIDTH as f64 * 0.7);

        let dy2 = y as f64 - (HEIGHT as f64 * 0.7);

        let r2 = (-(dx2 * dx2 + dy2 * dy2) / 30.0).exp();
    ...
    for step in 0..config.steps {
        for y in 1..HEIGHT - 1 {
            for x in 1..WIDTH - 1 {
                ...
                u_next[(x, y)] = 2.0 * u_curr[(x, y)] - u_prev[(x, y)] + s_x * lap_x + s_y * lap_y;
            }
        }
        ...
        if step % 20 == 0 {
            #[cfg(feature = "output")]
            {
                let frame_num = step / 20;

                let plot_path = format!("wave_surface_frame_{:02}.png", frame_num);

                let array = fdm_grid_to_ndarray(&u_curr);
                ...
                if let Err(e) = plot_surface_2d(&array, &plot_path, Some(plot_config)) {
...
fn fdm_grid_to_ndarray(grid: &FdmGrid<f64>) -> Array2<f64> {
    let (width, height) = match grid.dimensions() {
        | Dimensions::D2(w, h) => (*w, *h),
        | _ => {
