...
use ndarray::Array2;
use rssn::output::plotting::{plot_heatmap_2d, PlotConfig};
...
fn update_grid(grid: &Array2<f64>, alpha: f64, dt: f64, dx: f64, dy: f64) -> Array2<f64> {
...
fn main() {
    println!("=== 2D Heat Equation PDE Example ===\n");
    println!("NOTE: The rssn library currently contains a placeholder for a PDE solver.");
    println!("This example demonstrates a direct implementation of a finite difference method for the 2D heat equation.\n");
    ...
    // Plot initial state
    println!("Generating plot for initial state (t=0)...");
    let plot_config = PlotConfig {
        width: 800,
        height: 800,
        caption: "2D Heat Equation (t=0)".to_string(),
        ..Default::default()
    };
    if let Err(e) = plot_heatmap_2d(&grid, "heat_equation_step_0.png", Some(plot_config)) {
        eprintln!("Error generating plot: {}", e);
    } else {
        println!("Plot saved to heat_equation_step_0.png");
    }
    ...
    for step in 0..time_steps {
        ...
        if (step + 1) % 500 == 0 {
            ...
            println!("\nGenerating plot for state at t = {:.2}...", time);
