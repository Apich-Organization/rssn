...
use rssn::output::plotting::{plot_3d_path_from_points, PlotConfig};

fn main() {
    println!("=== Lorenz Attractor ODE Solver Example ===\n");
    ...
    // Let: y0 = x, y1 = y, y2 = z
    let ode_system_strs = [
        format!("{} * (y1 - y0)", sigma),      // dx/dt = sigma * (y - x)
        format!("y0 * ({} - y2) - y1", rho), // dy/dt = x * (rho - z) - y
        format!("y0 * y1 - {} * y2", beta),    // dz/dt = x * y - beta * z
    ];

    println!("Lorenz System of ODEs:");
    println!("  dx/dt = {}", ode_system_strs[0]);
    println!("  dy/dt = {}", ode_system_strs[1]);
    println!("  dz/dt = {}", ode_system_strs[2]);
    ...
    match solve_ode_system(&funcs, y0, x_range, num_steps, OdeSolverMethod::RungeKutta4) {
        Ok(results) => {
            println!("Successfully solved the ODE system.");
            println!("Showing the first 5 and last 5 results (of {} total):", results.len());

            // Print header
            println!("{:<5} | {:<20} | {:<20} | {:<20}", "Step", "x", "y", "z");
            println!("{:-<71}", "");
            ...
            for (i, res) in results.iter().rev().take(5).rev().enumerate() {
                let step = results.len() - 5 + i;
                println!(
                    "{:<5} | {:<20.8} | {:<20.8} | {:<20.8}",
                    step, res[0], res[1], res[2]
                );
            }

            // Plot the results
            println!("\nGenerating plot...");
            let plot_path = "lorenz_attractor.png";
            let plot_config = PlotConfig {
                caption: "Lorenz Attractor".to_string(),
                ..Default::default()
            };

            if let Err(e) = plot_3d_path_from_points(&results, plot_path, Some(plot_config)) {
        ...
        Err(e) => {
            eprintln!("Error solving ODE system: {}", e);
        }
    }
