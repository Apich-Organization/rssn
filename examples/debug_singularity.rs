...
fn main() {
    ...
    let factor = Expr::new_sub(z.clone(), singularity.clone());
    let factor_simplified = simplify(&factor);
    
    println!("Factor (z - 0): {:?}", factor);
    println!("Factor simplified: {:?}", factor_simplified);
    
    // Check what the denominator actually is
    if let Expr::Div(num, den) = &func {
        println!("Numerator: {:?}", num);
        println!("Denominator: {:?}", den);
        println!("Den == z? {}", den.as_ref() == &z);
        println!("Den == factor_simplified? {}", den.as_ref() == &factor_simplified);
    }
}
