...
pub(crate) fn parse_rational_structure(input: &str) -> IResult<&str, ()> {
    let (input, _) = nom_i64.parse(input)?;
...
pub(crate) fn comparison_expr(input: &str) -> IResult<&str, Expr> {
    let (input, init) = additive_expr.parse(input)?;
...
pub(crate) fn term(input: &str) -> IResult<&str, Expr> {
    let (input, init) = factor.parse(input)?;
...
pub(crate) fn factor(input: &str) -> IResult<&str, Expr> {
    delimited(multispace0, alt((unary, parenthesized_expr)), multispace0).parse(input)
}
...
pub(crate) fn unary(input: &str) -> IResult<&str, Expr> {
    ...
    let (input, not_op) = opt(preceded(tag("not"), multispace1)).parse(input)?;
    ...
    let (input, mut expr) = power.parse(input)?;
...
pub(crate) fn power(input: &str) -> IResult<&str, Expr> {
    let (input, base) = atom.parse(input)?;

    let (input, power_expr) = opt(preceded(char('^'), unary)).parse(input)?;

    let (input, factorial_op) = opt(char('!')).parse(input)?;
...
pub(crate) fn parse_bigint(input: &str) -> IResult<&str, Expr> {
    map(nom_i64, |n| Expr::BigInt(BigInt::from(n))).parse(input)
}
...
pub(crate) fn parse_rational(input: &str) -> IResult<&str, Expr> {
    ...
    // println!("{}", input);
    let (input, numerator) = nom_i64.parse(input)?;

    // println!("{}", numerator);
    let (input, denominator) = preceded(char('/'), nom_i64).parse(input)?;

    // println!("{}", denominator);
    let final_numerator = if sign.is_some() {
        // println!("matched -");
        // println!("{}", -numerator);
...
pub(crate) fn parse_infinity(input: &str) -> IResult<&str, Expr> {
    map(tag("Infinity"), |_| Expr::Infinity).parse(input)
}

pub(crate) fn parse_negative_infinity(input: &str) -> IResult<&str, Expr> {
    map(tag("-Infinity"), |_| Expr::NegativeInfinity).parse(input)
}
...
pub(crate) fn parse_numeric_literals(input: &str) -> IResult<&str, Expr> {
    // println!("in numeric");
    alt((parse_rational, parse_number, parse_bigint)).parse(input)
    // alt((parse_bigint, parse_number)).parse(input)
}

pub(crate) fn parse_boolean_and_infinities(input: &str) -> IResult<&str, Expr> {
    alt((parse_boolean, parse_infinity, parse_negative_infinity)).parse(input)
}
...
pub(crate) fn parse_function_call(input: &str) -> IResult<&str, Expr> {
    ...
    let (input, func_name) = identifier_name.parse(input)?;

    // println!("Parsed function name: {}", func_name);
    let (input, args) = delimited(
        char('('),
        nom::multi::separated_list0(delimited(multispace0, char(','), multispace0), expr),
        char(')'),
    )
    .parse(input)?;
...
pub(crate) fn parse_matrix(input: &str) -> IResult<&str, Expr> {
    let (input, _) = tag("matrix").parse(input)?;
...
pub(crate) fn parse_pde(input: &str) -> IResult<&str, Expr> {
    let (input, _) = tag("pde").parse(input)?;

    let (input, _) = char('(').parse(input)?;

    let (input, equation) = expr.parse(input)?;
...
pub(crate) fn parse_path(input: &str) -> IResult<&str, Expr> {
    ...
    let (input, path_type) = parse_path_type.parse(input)?;

    let (input, _) = delimited(multispace0, char(','), multispace0).parse(input)?;

    let (input, arg1) = expr.parse(input)?;

    let (input, _) = delimited(multispace0, char(','), multispace0).parse(input)?;

    let (input, arg2) = expr.parse(input)?;
...
pub(crate) fn parse_interval(input: &str) -> IResult<&str, Expr> {
    let (input, _) = tag("interval").parse(input)?;

    let (input, _) = char('(').parse(input)?;

    let (input, lower_bound) = expr.parse(input)?;

    let (input, _) = delimited(multispace0, char(','), multispace0).parse(input)?;

    let (input, upper_bound) = expr.parse(input)?;

    let (input, _) = delimited(multispace0, char(','), multispace0).parse(input)?;

    let (input, incl_lower) = parse_boolean.parse(input)?;

    let (input, _) = delimited(multispace0, char(','), multispace0).parse(input)?;

    let (input, incl_upper) = parse_boolean.parse(input)?;
...
pub(crate) fn parse_quantifier(input: &str) -> IResult<&str, Expr> {
    let (input, quantifier_type) = alt((tag("forall"), tag("exists"))).parse(input)?;

    let (input, _) = multispace1.parse(input)?;

    let (input, var_name) = alpha1.parse(input)?;
...
pub(crate) fn parse_domain(input: &str) -> IResult<&str, Expr> {
    let (input, _) = tag("domain").parse(input)?;

    let (input, domain_name) = delimited(char('('), alpha1, char(')')).parse(input)?;

    Ok((input, Expr::Domain(domain_name.to_string())))
}

pub(crate) fn parse_ode(input: &str) -> IResult<&str, Expr> {
    let (input, _) = tag("ode").parse(input)?;
...
pub(crate) fn parse_sum(input: &str) -> IResult<&str, Expr> {
    let (input, _) = tag("sum").parse(input)?;
    ...
    // println!("step4");
    // println!("{}", input);
    let (input, var) = expr.parse(input)?;
    ...
    let (input, from) = expr.parse(input)?;

    let (input, _) = delimited(multispace0, char(','), multispace0).parse(input)?;

    let (input, to) = expr.parse(input)?;
...
pub(crate) fn parse_integral(input: &str) -> IResult<&str, Expr> {
    let (input, _) = tag("integral").parse(input)?;

    let (input, _) = char('(').parse(input)?;

    let (input, integrand) = expr.parse(input)?;
...
pub(crate) fn parse_series_like_function(input: &str) -> IResult<&str, Expr> {
    let (input, func_name) = alt((tag("series"), tag("summation"), tag("product"))).parse(input)?;
    ...
    let (input, var_name_expr) = expr.parse(input)?;
    ...
    let (input, arg3) = expr.parse(input)?;

    let (input, _) = delimited(multispace0, char(','), multispace0).parse(input)?;

    let (input, arg4) = expr.parse(input)?;
...
pub(crate) fn parse_asymptotic_expansion(input: &str) -> IResult<&str, Expr> {
    ...
    let (input, _) = tag("asymptotic_expansion").parse(input)?;
...
pub(crate) fn parse_fredholm(input: &str) -> IResult<&str, Expr> {
    let (input, _) = tag("fredholm").parse(input)?;
...
pub(crate) fn parse_volterra(input: &str) -> IResult<&str, Expr> {
    let (input, _) = tag("volterra").parse(input)?;
...
pub(crate) fn parse_parametric_solution(input: &str) -> IResult<&str, Expr> {
    let (input, _) = tag("parametric_solution").parse(input)?;

    let (input, _) = char('(').parse(input)?;

    let (input, x_expr) = expr.parse(input)?;

    let (input, _) = delimited(multispace0, char(','), multispace0).parse(input)?;

    let (input, y_expr) = expr.parse(input)?;
...
pub(crate) fn parse_root_of(input: &str) -> IResult<&str, Expr> {
    let (input, _) = tag("root_of").parse(input)?;

    let (input, _) = char('(').parse(input)?;

    let (input, poly) = expr.parse(input)?;

    let (input, _) = delimited(multispace0, char(','), multispace0).parse(input)?;

    let (input, index_expr) = expr.parse(input)?;
...
pub(crate) fn parse_number(input: &str) -> IResult<&str, Expr> {
    map(parse_float, Expr::Constant).parse(input)
}
