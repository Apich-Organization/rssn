...
pub fn write_npy_file<P: AsRef<Path>>(
    ...
    Err("Feature 'output' is required for .npy support".to_string())
}
...
pub fn read_csv_file<P: AsRef<Path>>(
    ...
    for line_res in reader.lines() {
        ...
        if rows == 0 {
            ...
            return Err(
                "Inconsistent column count in CSV"
                    .to_string(),
            );
        }
...
pub fn write_json_file<P: AsRef<Path>>(
    ...
    serde_json::to_writer_pretty(file, arr)
        .map_err(|e| e.to_string())
}

/// Reads a 2D `ndarray::Array` from a JSON file.
pub fn read_json_file<P: AsRef<Path>>(
    filename: P,
) -> Result<Array2<f64>, String> {
    let file = File::open(filename).map_err(|e| e.to_string())?;
    let reader = BufReader::new(file);
...
pub fn save_expr_as_csv<P: AsRef<Path>>(
    ...
    if let Expr::Matrix(rows) = matrix_expr {
        ...
        for (i, row) in rows.iter().enumerate() {
            if row.len() != num_cols {
                return Err("Inconsistent columns".to_string());
            }
...
pub fn load_csv_as_expr<P: AsRef<Path>>(
    path: P,
) -> Result<Expr, String> {
...
pub fn load_json_as_expr<P: AsRef<Path>>(
    path: P,
) -> Result<Expr, String> {
...
pub fn load_npy_as_expr<P: AsRef<Path>>(
    path: P,
) -> Result<Expr, String> {
