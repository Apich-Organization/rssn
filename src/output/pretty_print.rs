...
pub(crate) fn to_box(root_expr: &Expr) -> PrintBox {
    let mut results: HashMap<Expr, PrintBox> = HashMap::new();
    ...
    while let Some(expr) = stack.last() {
        ...
        let all_children_processed = children.iter().all(|c| results.contains_key(c));

        if all_children_processed {
            let current_expr = stack.pop().expect("Value is valid");

            let get_child_box = |i: usize| -> &PrintBox { &results[&children[i]] };
            ...
            let val = match current_expr.op() {
                ...
                | DagOp::Neg => get_child_box(0).clone().prefix("-"),
                | DagOp::Vector => {
                ...
                | DagOp::Matrix { rows, cols } => {
                    ...
                    let mut grid: Vec<Vec<&PrintBox>> =
                        vec![vec![&results[&children[0]]; cols]; rows];
                    for r in 0..rows {
                        for c in 0..cols {
                        ...
                        for sub_row in 0..row_heights[r] {
                            ...
                            for c in 0..cols {
                                ...
                                let cell_line = cell
                                    ...
                                    .get(sub_row)
                                    .cloned()
                                    .unwrap_or_else(|| " ".repeat(cell.width));
                ...
                | DagOp::Binomial => {
                    ...
                    let mut lines = Vec::new();
                    for l in &n.lines {
                        lines.push(center_text(l, width));
                    }
                    for l in &k.lines {
                        lines.push(center_text(l, width));
                    }
                ...
                | DagOp::Factorial => get_child_box(0).clone().suffix("!"),
    ...
    results.remove(root_expr).expect("Remove Failed")
}
...
mod tests {
    ...
    fn test_pretty_print_factorial() {
        ...
        let expr = Expr::Factorial(std::sync::Arc::new(n));
