...
impl MaxwellEquations {
    ...
    pub fn new(e_field: &Vector, b_field: &Vector, rho: &Expr, j_field: &Vector, vars: (&str, &str, &str), t_var: &str) -> Self {
...
pub fn lorentz_force(charge: &Expr, e_field: &Vector, velocity: &Vector, b_field: &Vector) -> Vector {
...
pub fn electric_field_from_potentials(v: &Expr, a: &Vector, vars: (&str, &str, &str), t_var: &str) -> Vector {
...
pub fn poynting_vector(e_field: &Vector, b_field: &Vector) -> Vector {
    let mu_0 = Expr::new_variable("mu_0");
    e_field.cross(b_field).scalar_mul(&Expr::new_div(Expr::Constant(1.0), mu_0))
}
...
pub fn energy_density(e_field: &Vector, b_field: &Vector) -> Expr {
    ...
    let term_e = Expr::new_mul(epsilon_0, Expr::new_pow(e_field.magnitude(), Expr::Constant(2.0)));
