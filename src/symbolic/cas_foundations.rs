...
pub fn normalize(expr: Expr) -> Expr {
    match expr {
        | Expr::Dag(node) => normalize(node.to_expr().expect("Normalize")),
        | Expr::Add(a, b) => {
    ...
    let expanded_expr = match expr {
        ...
        | Expr::Mul(a, b) => {
            ...
            let exp_a_unwrapped = match exp_a {
                | Expr::Dag(ref node) => node.to_expr().unwrap_or_else(|_| exp_a.clone()),
                | _ => exp_a,
            };

            let exp_b_unwrapped = match exp_b {
                | Expr::Dag(ref node) => node.to_expr().unwrap_or_else(|_| exp_b.clone()),
                | _ => exp_b,
            };
        ...
        | Expr::Power(base, exp) => {
            ...
            if let Some(n) = exp_exp.to_f64() {
                if n.fract() == 0.0 && n > 1.0 {
                    let n_us = (n as i64).try_into().unwrap_or(0);
...
pub(crate) fn factorize_terms(terms: Vec<Expr>) -> Expr {
