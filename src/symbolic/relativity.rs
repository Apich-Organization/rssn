...
pub fn relativistic_momentum(mass: &Expr, velocity: &Expr) -> Expr {
    ...
    Expr::new_mul(
        gamma,
        Expr::new_mul(mass.clone(), velocity.clone())
    )
}
...
pub fn doppler_effect(f_src: &Expr, v: &Expr) -> Expr {
    ...
    Expr::new_mul(
        f_src.clone(),
        Expr::new_pow(ratio, Expr::Constant(0.5))
    )
}
...
pub fn einstein_field_equations(ricci: &Expr, scalar: &Expr, metric: &Expr, _stress: &Expr) -> Expr {
