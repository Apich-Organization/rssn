    ...
    pub fn continue_along_path(&mut self, path_points: &[Expr]) -> Result<(), String> {
        for next_point in path_points {
            ...
            let radius = estimate_radius_of_convergence(
                ...
                .ok_or_else(|| {
                    "Failed to estimate the radius of convergence.".to_string()
                })?;
            ...
            if distance >= radius {
                return Err(
                    format!(
                        "Analytic continuation failed: point {} is outside radius {} of series at {}.",
...
impl MobiusTransformation {
    ...
    pub fn new(a: Expr, b: Expr, c: Expr, d: Expr) -> Self {
    ...
    pub fn apply(&self, z: &Expr) -> Expr {
        let numerator = Expr::new_add(
            Expr::new_mul(self.a.clone(), z.clone()),
            self.b.clone()
        );
        let denominator = Expr::new_add(
            Expr::new_mul(self.c.clone(), z.clone()),
            self.d.clone()
        );
