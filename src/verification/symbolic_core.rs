...
#[kani::proof]
fn proof_new_constant_is_dag() {
    let c: f64 = kani::any();
    if c.is_finite() {
        let expr = Expr::new_constant(c);
        assert!(expr.is_dag());
    }
}
...
fn proof_new_bigint_is_dag() {
    ...
    let i: i64 = kani::any();
    let expr = Expr::new_bigint(BigInt::from(i));
    assert!(expr.is_dag());
}
...
fn proof_new_rational_is_dag() {
    ...
    let d: i64 = kani::any();
    if d != 0 {
        let expr = Expr::new_rational(BigRational::new(BigInt::from(n), BigInt::from(d)));
        assert!(expr.is_dag());
    }
}
...
fn proof_add_commutativity_normalization() {
    ...
    assert_eq!(sum1, sum2, "Addition should be normalized (commutative)");
}
...
fn proof_mul_commutativity_normalization() {
    ...
    assert_eq!(prod1, prod2, "Multiplication should be normalized (commutative)");
}
...
fn proof_nested_add_normalization() {
    ...
    let sum1 = Expr::new_add(&Expr::new_add(&x, &y), &z);
    let sum2 = Expr::new_add(&x, &Expr::new_add(&y, &z));
...
fn proof_to_dag_idempotence() {
    let c: f64 = kani::any();
    if c.is_finite() {
        let ast_expr = Expr::Constant(c);
        let dag_expr_1 = ast_expr.to_dag().expect("to_dag failed");
        assert!(dag_expr_1.is_dag());
        
        let dag_expr_2 = dag_expr_1.to_dag().expect("to_dag failed again");
        assert_eq!(dag_expr_1, dag_expr_2);
    }
}
...
fn proof_to_ast_to_dag_roundtrip() {
    ...
    if c.is_finite() {
        let expr = Expr::new_constant(c);
        let ast = expr.to_ast().expect("to_ast failed");
        let dag = ast.to_dag().expect("to_dag failed");
        assert_eq!(expr, dag);
    }
}
...
fn proof_vector_is_dag() {
    let x = Expr::new_variable("x");
    let y = Expr::new_variable("y");
    let vec_expr = Expr::new_vector(vec![x, y]);
...
fn proof_tuple_is_dag() {
    let x = Expr::new_variable("x");
    let y = Expr::new_variable("y");
    let tuple_expr = Expr::new_tuple(vec![x, y]);
...
fn proof_logic_ops_is_dag() {
    ...
    assert!(Expr::new_and(vec![x.clone(), y.clone()]).is_dag());
    assert!(Expr::new_or(vec![x.clone(), y.clone()]).is_dag());
    assert!(Expr::new_not(&x).is_dag());
    assert!(Expr::new_xor(&x, &y).is_dag());
    assert!(Expr::new_implies(&x, &y).is_dag());
    assert!(Expr::new_equivalent(&x, &y).is_dag());
}
...
fn proof_quantifiers_is_dag() {
    let x = Expr::new_variable("x");
    let body = Expr::new_gt(&x, &Expr::new_constant(0.0));
    
    assert!(Expr::new_forall("x", &body).is_dag());
    assert!(Expr::new_exists("x", &body).is_dag());
}
...
fn proof_interval_is_dag() {
    ...
    let interval = Expr::new_interval(&l, &u, true, false);
