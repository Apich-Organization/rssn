...
unsafe fn collect_pairs(x_data: *const *const Expr, y_data: *const *const Expr, len: usize) -> Vec<(Expr, Expr)> {
...
pub unsafe extern "C" fn rssn_simple_linear_regression(
    ...
    if x_data.is_null() || y_data.is_null() { return std::ptr::null_mut(); }
...
pub unsafe extern "C" fn rssn_nonlinear_regression(
    ...
    if x_data.is_null() || y_data.is_null() || model.is_null() { return std::ptr::null_mut(); }
    let data = collect_pairs(x_data, y_data, len);
    ...
    match stats_regression::nonlinear_regression_symbolic(&data, model_expr, &vars_vec, &params_vec) {
        Some(solutions) => {
            let eqs = solutions.into_iter().map(|(p, v)| Expr::Eq(Arc::new(p), Arc::new(v))).collect();
