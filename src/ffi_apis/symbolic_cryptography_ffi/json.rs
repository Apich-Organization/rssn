...
pub unsafe extern "C" fn rssn_json_elliptic_curve_new(
    ...
    if let (Some(a), Some(b), Some(m)) = (parse_bigint(a_str), parse_bigint(b_str), parse_bigint(mod_str)) {
...
pub unsafe extern "C" fn rssn_json_curve_point_affine(
    ...
    if let (Some(x), Some(y), Some(m)) = (parse_bigint(x_str), parse_bigint(y_str), parse_bigint(mod_str)) {
...
pub unsafe extern "C" fn rssn_json_ecdsa_sign(
    ...
    if let (Some(h), Some(pk), Some(c), Some(g), Some(o)) = 
        (parse_bigint(hash_str), parse_bigint(pk_str), curve, gen, parse_bigint(order_str)) {
...
pub unsafe extern "C" fn rssn_json_ecdsa_verify(
    ...
    if let (Some(h), Some(sig), Some(pk), Some(c), Some(g), Some(o)) = 
        (parse_bigint(hash_str), sig, pub_key, curve, gen, parse_bigint(order_str)) {
