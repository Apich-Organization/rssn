...
#[no_mangle]
pub unsafe extern "C" fn rssn_bincode_graph_is_connected(graph_buf: BincodeBuffer) -> BincodeBuffer {
    let graph: Graph<String> = match from_bincode_buffer(&graph_buf) {
        Some(g) => g,
        None => return BincodeBuffer::empty(),
    };
...
#[no_mangle]
pub unsafe extern "C" fn rssn_bincode_graph_strongly_connected_components(graph_buf: BincodeBuffer) -> BincodeBuffer {
    let graph: Graph<String> = match from_bincode_buffer(&graph_buf) {
        Some(g) => g,
        None => return BincodeBuffer::empty(),
    };
...
#[no_mangle]
pub unsafe extern "C" fn rssn_bincode_graph_bridges_and_articulation_points(graph_buf: BincodeBuffer) -> BincodeBuffer {
    let graph: Graph<String> = match from_bincode_buffer(&graph_buf) {
        Some(g) => g,
        None => return BincodeBuffer::empty(),
    };
...
#[no_mangle]
pub unsafe extern "C" fn rssn_bincode_graph_edmonds_karp_max_flow(input_buf: BincodeBuffer) -> BincodeBuffer {
    #[derive(serde::Deserialize)]
    struct Input {
        graph: Graph<String>,
        source: usize,
...
#[no_mangle]
pub unsafe extern "C" fn rssn_bincode_graph_dinic_max_flow(input_buf: BincodeBuffer) -> BincodeBuffer {
    #[derive(serde::Deserialize)]
    struct Input {
        graph: Graph<String>,
        source: usize,
...
#[no_mangle]
pub unsafe extern "C" fn rssn_bincode_graph_bipartite_maximum_matching(input_buf: BincodeBuffer) -> BincodeBuffer {
    #[derive(serde::Deserialize)]
    struct Input {
        graph: Graph<String>,
        partition: Vec<i8>,
...
#[no_mangle]
pub unsafe extern "C" fn rssn_bincode_graph_topological_sort(graph_buf: BincodeBuffer) -> BincodeBuffer {
    let graph: Graph<String> = match from_bincode_buffer(&graph_buf) {
        Some(g) => g,
        None => return BincodeBuffer::empty(),
    };
