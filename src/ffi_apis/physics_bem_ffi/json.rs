...
#[no_mangle]
pub unsafe extern "C" fn rssn_physics_bem_solve_laplace_2d_json(input: *const c_char) -> *mut c_char {
    let input: Bem2DInput = match from_json_string(input) {
        Some(i) => i,
        None => return to_c_string(serde_json::to_string(&FfiResult::<Bem2DOutput, String>::err("Invalid JSON".to_string())).unwrap()),
    };
    ...
    match physics_bem::solve_laplace_bem_2d(&input.points, &bcs) {
        Ok((u, q)) => to_c_string(serde_json::to_string(&FfiResult::<Bem2DOutput, String>::ok(Bem2DOutput { u, q })).unwrap()),
        Err(e) => to_c_string(serde_json::to_string(&FfiResult::<Bem2DOutput, String>::err(e)).unwrap()),
    }
}
