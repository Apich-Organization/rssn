...
#[no_mangle]
pub unsafe extern "C" fn rssn_physics_sim_ising_run_bincode(buffer: BincodeBuffer) -> BincodeBuffer {
    let params: IsingParameters = match from_bincode_buffer(&buffer) {
        Some(p) => p,
        None => return to_bincode_buffer(&FfiResult::<IsingOutput, String>::err("Invalid Bincode".to_string())),
    };
