...
pub(crate) fn convert_expr_graph_to_string_graph(g: Graph<Expr>) -> Graph<String> {
    ...
    for (u, v, weight) in g.get_edges() {
        let u_new = id_map[&u];
        let v_new = id_map[&v];
        new_graph.add_edge(&new_graph.nodes[u_new].clone(), &new_graph.nodes[v_new].clone(), weight);
    }
...
pub extern "C" fn rssn_graph_union(
    ...
    let g1 = unsafe { &*(ptr1 as *const Graph<String>) };
    let g2 = unsafe { &*(ptr2 as *const Graph<String>) };
