...
pub unsafe extern "C" fn rssn_num_graph_dijkstra_json(input_json: *const c_char) -> *mut c_char {
    let input: DijkstraInput = match from_json_string(input_json) {
        Some(i) => i,
        None => return to_c_string(serde_json::to_string(&FfiResult::<DijkstraOutput, String> { ok: None, err: Some("Invalid JSON input".to_string()) }).unwrap()),
    };
    
    let g = input.graph.to_graph();
    let (dist, prev) = dijkstra(&g, input.start_node);
    
    to_c_string(serde_json::to_string(&FfiResult { 
...
pub unsafe extern "C" fn rssn_num_graph_bfs_json(input_json: *const c_char) -> *mut c_char {
    let input: DijkstraInput = match from_json_string(input_json) {
        Some(i) => i,
        None => return to_c_string(serde_json::to_string(&FfiResult::<Vec<usize>, String> { ok: None, err: Some("Invalid JSON input".to_string()) }).unwrap()),
    };
    
    let g = input.graph.to_graph();
    let dist = bfs(&g, input.start_node);
    
    to_c_string(serde_json::to_string(&FfiResult { ok: Some(dist), err: None::<String> }).unwrap())
}
...
pub unsafe extern "C" fn rssn_num_graph_page_rank_json(input_json: *const c_char) -> *mut c_char {
    ...
    to_c_string(serde_json::to_string(&FfiResult { ok: Some(scores), err: None::<String> }).unwrap())
}

#[no_mangle]
pub unsafe extern "C" fn rssn_num_graph_floyd_warshall_json(input_json: *const c_char) -> *mut c_char {
    ...
    to_c_string(serde_json::to_string(&FfiResult { ok: Some(mat), err: None::<String> }).unwrap())
}
...
#[no_mangle]
pub unsafe extern "C" fn rssn_num_graph_connected_components_json(input_json: *const c_char) -> *mut c_char {
    let input: GraphDef = match from_json_string(input_json) {
        Some(i) => i,
        None => return to_c_string(serde_json::to_string(&FfiResult::<Vec<usize>, String> { ok: None, err: Some("Invalid JSON input".to_string()) }).unwrap()),
    };
    ...
    to_c_string(serde_json::to_string(&FfiResult { ok: Some(comp), err: None::<String> }).unwrap())
}

#[no_mangle]
pub unsafe extern "C" fn rssn_num_graph_minimum_spanning_tree_json(input_json: *const c_char) -> *mut c_char {
    let input: GraphDef = match from_json_string(input_json) {
        Some(i) => i,
        None => return to_c_string(serde_json::to_string(&FfiResult::<GraphDefOut, String> { ok: None, err: Some("Invalid JSON input".to_string()) }).unwrap()),
    };
    ...
    to_c_string(serde_json::to_string(&FfiResult { ok: Some(mst_def), err: None::<String> }).unwrap())
}
