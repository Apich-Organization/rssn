...
#[no_mangle]
pub unsafe extern "C" fn rssn_physics_mm_sph_update_bincode(buffer: BincodeBuffer) -> BincodeBuffer {
    let mut input: SphInput = match from_bincode_buffer(&buffer) {
        Some(i) => i,
        None => return to_bincode_buffer(&FfiResult::<SPHSystem, String>::err("Invalid Bincode".to_string())),
    };
