...
#[no_mangle]
pub unsafe extern "C" fn rssn_physics_sim_schrodinger_run_json(input: *const c_char) -> *mut c_char {
    ...
    match schrodinger_quantum::run_schrodinger_simulation(&input.params, &mut initial_psi) {
        Ok(snapshots) => {
            if let Some(final_state) = snapshots.last() {
                to_c_string(serde_json::to_string(&FfiResult::<Vec<f64>, String>::ok(final_state.clone().into_raw_vec())).unwrap())
            } else {
                to_c_string(serde_json::to_string(&FfiResult::<Vec<f64>, String>::err("No snapshots produced".to_string())).unwrap())
            }
        }
