...
pub extern "C" fn rssn_json_kinetic_energy(
    ...
    match (mass, velocity) {
        | (Some(m), Some(v)) => to_json_string(&classical_mechanics::kinetic_energy(&m, &v)),
        | _ => std::ptr::null_mut(),
    }
}
...
pub extern "C" fn rssn_json_euler_lagrange_equation(
    ...
    match (lagrangian, q_str, q_dot_str, t_str) {
        | (Some(l), Some(qs), Some(qds), Some(ts)) => {
            to_json_string(&classical_mechanics::euler_lagrange_equation(
                &l, qs, qds, ts,
            ))
        },
        | _ => std::ptr::null_mut(),
