...
pub unsafe extern "C" fn rssn_num_dg_metric_tensor_json(input_json: *const c_char) -> *mut c_char {
    ...
    match differential_geometry::metric_tensor_at_point(input.system, &input.point) {
        ...
        Err(e) => {
            let ffi_res = FfiResult { ok: None::<Vec<Vec<f64>>>, err: Some(e) };
            to_c_string(serde_json::to_string(&ffi_res).unwrap())
        }
    }
}
...
#[no_mangle]
pub unsafe extern "C" fn rssn_num_dg_christoffel_symbols_json(input_json: *const c_char) -> *mut c_char {
    let input: DgPointInput = match from_json_string(input_json) {
        Some(i) => i,
        None => return to_c_string(serde_json::to_string(&FfiResult::<Vec<Vec<Vec<f64>>>, String> { ok: None, err: Some("Invalid JSON input".to_string()) }).unwrap()),
    };
    ...
    match differential_geometry::christoffel_symbols(input.system, &input.point) {
        ...
        Err(e) => {
            let ffi_res = FfiResult { ok: None::<Vec<Vec<Vec<f64>>>>, err: Some(e) };
            to_c_string(serde_json::to_string(&ffi_res).unwrap())
        }
    }
}
