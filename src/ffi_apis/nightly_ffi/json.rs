...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rssn_num_matrix_add_json_nightly(json_ptr: *const c_char) -> *mut c_char {
    if json_ptr.is_null() {
...
/// or if internal state synchronization fails (e.g., poisoned locks).
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rssn_num_matrix_mul_json_nightly(json_ptr: *const c_char) -> *mut c_char {
    if json_ptr.is_null() {
        return std::ptr::null_mut();
    }
...
/// or if internal state synchronization fails (e.g., poisoned locks).
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rssn_num_matrix_det_json_nightly(json_ptr: *const c_char) -> *mut c_char {
    if json_ptr.is_null() {
        return std::ptr::null_mut();
    }
