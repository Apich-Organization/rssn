...
pub unsafe extern "C" fn rssn_num_comb_factorial_bincode(buffer: BincodeBuffer) -> BincodeBuffer {
    ...
    to_bincode_buffer(&FfiResult { ok: Some(res), err: None::<String> })
}

#[no_mangle]
pub unsafe extern "C" fn rssn_num_comb_permutations_bincode(buffer: BincodeBuffer) -> BincodeBuffer {
...
#[no_mangle]
pub unsafe extern "C" fn rssn_num_comb_combinations_bincode(buffer: BincodeBuffer) -> BincodeBuffer {
    let input: NKInput = match from_bincode_buffer(&buffer) {
        Some(i) => i,
        None => return to_bincode_buffer(&FfiResult::<f64, String> { ok: None, err: Some("Invalid Bincode input".to_string()) }),
    };
...
#[no_mangle]
pub unsafe extern "C" fn rssn_num_comb_solve_recurrence_bincode(buffer: BincodeBuffer) -> BincodeBuffer {
    let input: RecurrenceInput = match from_bincode_buffer(&buffer) {
        Some(i) => i,
        None => return to_bincode_buffer(&FfiResult::<f64, String> { ok: None, err: Some("Invalid Bincode input".to_string()) }),
    };
    match combinatorics::solve_recurrence_numerical(&input.coeffs, &input.initial_conditions, input.target_n) {
        Ok(res) => to_bincode_buffer(&FfiResult { ok: Some(res), err: None::<String> }),
        Err(e) => to_bincode_buffer(&FfiResult::<f64, String> { ok: None, err: Some(e) }),
    }
}
...
#[no_mangle]
pub unsafe extern "C" fn rssn_num_comb_stirling_second_bincode(buffer: BincodeBuffer) -> BincodeBuffer {
    let input: NKInput = match from_bincode_buffer(&buffer) {
        Some(i) => i,
        None => return to_bincode_buffer(&FfiResult::<f64, String> { ok: None, err: Some("Invalid Bincode input".to_string()) }),
    };
...
#[no_mangle]
pub unsafe extern "C" fn rssn_num_comb_rising_factorial_bincode(buffer: BincodeBuffer) -> BincodeBuffer {
...
#[no_mangle]
pub unsafe extern "C" fn rssn_num_comb_falling_factorial_bincode(buffer: BincodeBuffer) -> BincodeBuffer {
    let input: XNInput = match from_bincode_buffer(&buffer) {
        Some(i) => i,
        None => return to_bincode_buffer(&FfiResult::<f64, String> { ok: None, err: Some("Invalid Bincode input".to_string()) }),
    };
