...
pub unsafe extern "C" fn rssn_polygamma(n: *const Expr, z: *const Expr) -> *mut Expr {
    if n.is_null() || z.is_null() { return std::ptr::null_mut(); }
    Box::into_raw(Box::new(special_functions::polygamma((*n).clone(), (*z).clone())))
}
...
pub unsafe extern "C" fn rssn_bessel_i(order: *const Expr, arg: *const Expr) -> *mut Expr {
    if order.is_null() || arg.is_null() { return std::ptr::null_mut(); }
    Box::into_raw(Box::new(special_functions::bessel_i((*order).clone(), (*arg).clone())))
}
...
pub unsafe extern "C" fn rssn_bessel_k(order: *const Expr, arg: *const Expr) -> *mut Expr {
    if order.is_null() || arg.is_null() { return std::ptr::null_mut(); }
    Box::into_raw(Box::new(special_functions::bessel_k((*order).clone(), (*arg).clone())))
}
...
pub unsafe extern "C" fn rssn_generalized_laguerre(
    ...
    if n.is_null() || alpha.is_null() || x.is_null() { return std::ptr::null_mut(); }
    Box::into_raw(Box::new(special_functions::generalized_laguerre(
        (*n).clone(), (*alpha).clone(), (*x).clone()
    )))
}
...
pub unsafe extern "C" fn rssn_chebyshev_t(n: *const Expr, x: *const Expr) -> *mut Expr {
    ...
    Box::into_raw(Box::new(special_functions::chebyshev_t((*n).clone(), (*x).clone())))
}
...
pub unsafe extern "C" fn rssn_chebyshev_u(n: *const Expr, x: *const Expr) -> *mut Expr {
    if n.is_null() || x.is_null() { return std::ptr::null_mut(); }
    Box::into_raw(Box::new(special_functions::chebyshev_u((*n).clone(), (*x).clone())))
}
...
pub unsafe extern "C" fn rssn_chebyshev_differential_equation(
    ...
    if y.is_null() || x.is_null() || n.is_null() { return std::ptr::null_mut(); }
    Box::into_raw(Box::new(special_functions::chebyshev_differential_equation(&*y, &*x, &*n)))
}
