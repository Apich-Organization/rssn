...
#[no_mangle]
pub extern "C" fn rssn_bincode_polygamma(n_buf: BincodeBuffer, z_buf: BincodeBuffer) -> BincodeBuffer {
...
#[no_mangle]
pub extern "C" fn rssn_bincode_bessel_i(order_buf: BincodeBuffer, arg_buf: BincodeBuffer) -> BincodeBuffer {
    let order: Option<Expr> = from_bincode_buffer(&order_buf);
    let arg: Option<Expr> = from_bincode_buffer(&arg_buf);
    if let (Some(o), Some(a)) = (order, arg) {
...
#[no_mangle]
pub extern "C" fn rssn_bincode_bessel_k(order_buf: BincodeBuffer, arg_buf: BincodeBuffer) -> BincodeBuffer {
    let order: Option<Expr> = from_bincode_buffer(&order_buf);
    let arg: Option<Expr> = from_bincode_buffer(&arg_buf);
    if let (Some(o), Some(a)) = (order, arg) {
...
#[no_mangle]
pub extern "C" fn rssn_bincode_chebyshev_t(n_buf: BincodeBuffer, x_buf: BincodeBuffer) -> BincodeBuffer {
    let n: Option<Expr> = from_bincode_buffer(&n_buf);
    let x: Option<Expr> = from_bincode_buffer(&x_buf);
...
#[no_mangle]
pub extern "C" fn rssn_bincode_chebyshev_u(n_buf: BincodeBuffer, x_buf: BincodeBuffer) -> BincodeBuffer {
    let n: Option<Expr> = from_bincode_buffer(&n_buf);
    let x: Option<Expr> = from_bincode_buffer(&x_buf);
    if let (Some(n), Some(x)) = (n, x) {
...
#[no_mangle]
pub extern "C" fn rssn_bincode_chebyshev_differential_equation(y_buf: BincodeBuffer, x_buf: BincodeBuffer, n_buf: BincodeBuffer) -> BincodeBuffer {
    let y: Option<Expr> = from_bincode_buffer(&y_buf);
    let x: Option<Expr> = from_bincode_buffer(&x_buf);
    let n: Option<Expr> = from_bincode_buffer(&n_buf);
    if let (Some(y), Some(x), Some(n)) = (y, x, n) {
