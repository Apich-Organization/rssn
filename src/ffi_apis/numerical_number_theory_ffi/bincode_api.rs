...
#[no_mangle]
pub unsafe extern "C" fn rssn_num_nt_factorize_bincode(data: *const u8, len: usize) -> BincodeBuffer {
    let req: FactorizeRequest = match decode(data, len) {
        Some(r) => r,
        None => return encode(&FfiResult::<Vec<u64>, String> { ok: None, err: Some("Bincode decode error".to_string()) }),
    };

    let factors = nt::factorize(req.n);
    encode(&FfiResult::<Vec<u64>, String> { ok: Some(factors), err: None })
}
