...
pub unsafe extern "C" fn rssn_num_stats_mean_json(input: *const c_char) -> *mut c_char {
    ...
    to_c_string(serde_json::to_string(&FfiResult { ok: Some(result), err: None::<String> }).unwrap())
}
...
#[no_mangle]
pub unsafe extern "C" fn rssn_num_stats_two_sample_t_test_json(input: *const c_char) -> *mut c_char {
    let input: TwoDataInput = match from_json_string(input) {
        Some(i) => i,
        None => return to_c_string(serde_json::to_string(&FfiResult::<TestOutput, String> { ok: None, err: Some("Invalid JSON".to_string()) }).unwrap()),
    };
    let (t, p) = stats::two_sample_t_test(&input.data1, &input.data2);
    to_c_string(serde_json::to_string(&FfiResult { ok: Some(TestOutput { statistic: t, p_value: p }), err: None::<String> }).unwrap())
}
...
#[no_mangle]
pub unsafe extern "C" fn rssn_num_stats_chi_squared_test_json(input: *const c_char) -> *mut c_char {
    let input: TwoDataInput = match from_json_string(input) {
        Some(i) => i,
        None => return to_c_string(serde_json::to_string(&FfiResult::<TestOutput, String> { ok: None, err: Some("Invalid JSON".to_string()) }).unwrap()),
    };
    let (chi, p) = stats::chi_squared_test(&input.data1, &input.data2);
    to_c_string(serde_json::to_string(&FfiResult { ok: Some(TestOutput { statistic: chi, p_value: p }), err: None::<String> }).unwrap())
}

#[no_mangle]
pub unsafe extern "C" fn rssn_num_stats_linear_regression_json(input: *const c_char) -> *mut c_char {
    let input: RegressionInput = match from_json_string(input) {
        Some(i) => i,
        None => return to_c_string(serde_json::to_string(&FfiResult::<RegressionOutput, String> { ok: None, err: Some("Invalid JSON".to_string()) }).unwrap()),
    };
    ...
    to_c_string(serde_json::to_string(&FfiResult { ok: Some(RegressionOutput { slope, intercept }), err: None::<String> }).unwrap())
}
...
pub unsafe extern "C" fn rssn_num_stats_z_scores_json(input: *const c_char) -> *mut c_char {
    let input: DataInput = match from_json_string(input) {
        Some(i) => i,
        None => return to_c_string(serde_json::to_string(&FfiResult::<Vec<f64>, String> { ok: None, err: Some("Invalid JSON".to_string()) }).unwrap()),
    };
