...
pub unsafe extern "C" fn rssn_json_verify_equation_solution(
    ...
    match (equations, solution, free_vars) {
        | (Some(eqs), Some(sol), Some(free)) => {
            let free_refs: Vec<&str> = free.iter().map(std::string::String::as_str).collect();
...
pub unsafe extern "C" fn rssn_json_verify_indefinite_integral(
    ...
    match (integrand, integral_result, var) {
        | (Some(f), Some(int), Some(v)) => proof::verify_indefinite_integral(&f, &int, &v),
        | _ => false,
    }
}
...
pub unsafe extern "C" fn rssn_json_verify_matrix_inverse(
    ...
    match (original, inverse) {
        | (Some(orig), Some(inv)) => proof::verify_matrix_inverse(&orig, &inv),
        | _ => false,
    }
}
