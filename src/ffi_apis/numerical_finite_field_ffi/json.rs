...
pub unsafe extern "C" fn rssn_num_ff_pfe_add_json(json_ptr: *const c_char) -> *mut c_char {
    ...
    let req: PfeBinaryOpRequest = match serde_json::from_str(json_str) {
        Ok(r) => r,
        Err(e) => {
            let res: FfiResult<PrimeFieldElement, String> = FfiResult { ok: None, err: Some(e.to_string()) };
            return CString::new(serde_json::to_string(&res).unwrap()).unwrap().into_raw();
        }
    };

    let res_pfe = req.a + req.b;
    let ffi_res: FfiResult<PrimeFieldElement, String> = FfiResult { ok: Some(res_pfe), err: None };
...
pub unsafe extern "C" fn rssn_num_ff_gf256_mul_json(json_ptr: *const c_char) -> *mut c_char {
    ...
    let req: Gf256OpRequest = match serde_json::from_str(json_str) {
        Ok(r) => r,
        Err(e) => {
            let res: FfiResult<u8, String> = FfiResult { ok: None, err: Some(e.to_string()) };
            return CString::new(serde_json::to_string(&res).unwrap()).unwrap().into_raw();
        }
    };

    let res = finite_field::gf256_mul(req.a, req.b);
    let ffi_res: FfiResult<u8, String> = FfiResult { ok: Some(res), err: None };
    CString::new(serde_json::to_string(&ffi_res).unwrap()).unwrap().into_raw()
}
