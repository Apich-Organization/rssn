...
pub extern "C" fn rssn_json_expectation_value(
    ...
    match (op, psi) {
        | (Some(op), Some(psi)) => to_json_string(&quantum_mechanics::expectation_value(&op, &psi)),
        | _ => std::ptr::null_mut(),
    }
}
...
pub extern "C" fn rssn_json_uncertainty(
    ...
    match (op, psi) {
        | (Some(op), Some(psi)) => to_json_string(&quantum_mechanics::uncertainty(&op, &psi)),
        | _ => std::ptr::null_mut(),
    }
}
...
pub extern "C" fn rssn_json_bra_ket(
    ...
    match (bra, ket) {
        | (Some(bra), Some(ket)) => to_json_string(&quantum_mechanics::bra_ket(&bra, &ket)),
        | _ => std::ptr::null_mut(),
    }
}
