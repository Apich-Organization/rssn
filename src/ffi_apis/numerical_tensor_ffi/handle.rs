...
pub unsafe extern "C" fn rssn_num_tensor_create(
    ...
    let s = unsafe { std::slice::from_raw_parts(shape, ndim) };
...
pub unsafe extern "C" fn rssn_num_tensor_tensordot(
    ...
    let ta = unsafe { &*a };
    let tb = unsafe { &*b };
    let aa = unsafe { std::slice::from_raw_parts(axes_a, axes_a_len) };
    let ab = unsafe { std::slice::from_raw_parts(axes_b, axes_b_len) };
