...
#[no_mangle]
pub unsafe extern "C" fn rssn_num_tensor_tensordot_json(json_ptr: *const c_char) -> *mut c_char {
    ...
    let req: TensordotRequest = match serde_json::from_str(json_str) {
        Ok(r) => r,
        Err(e) => {
            let res: FfiResult<TensorData, String> = FfiResult { ok: None, err: Some(e.to_string()) };
    ...
    let a = match req.a.to_arrayd() {
        Ok(arr) => arr,
        Err(e) => {
            let res: FfiResult<TensorData, String> = FfiResult { ok: None, err: Some(e) };
            return CString::new(serde_json::to_string(&res).unwrap()).unwrap().into_raw();
        }
    };
    ...
    match tensor::tensordot(&a, &b, &req.axes_a, &req.axes_b) {
        Ok(res) => {
            let ffi_res: FfiResult<TensorData, String> = FfiResult { ok: Some(TensorData::from(&res)), err: None };
            CString::new(serde_json::to_string(&ffi_res).unwrap()).unwrap().into_raw()
        }
        Err(e) => {
            let ffi_res: FfiResult<TensorData, String> = FfiResult { ok: None, err: Some(e) };
            CString::new(serde_json::to_string(&ffi_res).unwrap()).unwrap().into_raw()
        }
    }
}
...
#[no_mangle]
pub unsafe extern "C" fn rssn_num_tensor_outer_product_json(json_ptr: *const c_char) -> *mut c_char {
    if json_ptr.is_null() { return std::ptr::null_mut(); }
    let json_str = match unsafe { CStr::from_ptr(json_ptr).to_str() } {
        Ok(s) => s,
        Err(_) => return std::ptr::null_mut(),
