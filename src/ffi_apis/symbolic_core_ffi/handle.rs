...
pub unsafe extern "C" fn rssn_core_create_derivative(
    ...
    unsafe {
        ...
        Box::into_raw(Box::new(Expr::new_derivative(
            expr_ref,
            var_str.to_string(),
        )))
    }
...
pub unsafe extern "C" fn rssn_core_create_derivativen(
    ...
    unsafe {
        if function.is_null() || grades.is_null() {
            return std::ptr::null_mut();
        }
...
pub unsafe extern "C" fn rssn_core_create_interval(
    ...
    unsafe {
        ...
        Box::into_raw(Box::new(Expr::new_interval(
            &*lower, &*upper, incl_lower, incl_upper,
        )))
    }
}
...
/// 3. Any pointers returned by this function are managed according to the API's ownership rules.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rssn_core_create_variable(name: *const c_char) -> *mut Expr {
    unsafe {
        let name_str = match c_str_to_str(name) {
            | Some(s) => s,
            | None => return std::ptr::null_mut(),
        ...
        Box::into_raw(Box::new(Expr::new_variable(name_str)))
    }
}
...
/// 3. Any pointers returned by this function are managed according to the API's ownership rules.
#[unsafe(no_mangle)]
pub extern "C" fn rssn_core_create_constant(val: f64) -> *mut Expr {
    Box::into_raw(Box::new(Expr::new_constant(val)))
}
...
/// 3. Any pointers returned by this function are managed according to the API's ownership rules.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rssn_core_free_expr(expr: *mut Expr) {
    unsafe {
        if !expr.is_null() {
...
/// 3. Any pointers returned by this function are managed according to the API's ownership rules.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rssn_core_to_string(expr: *const Expr) -> *mut c_char {
    unsafe {
        if expr.is_null() {
            return std::ptr::null_mut();
        }
...
/// 3. Any pointers returned by this function are managed according to the API's ownership rules.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn rssn_core_free_string(s: *mut c_char) {
    unsafe {
        if !s.is_null() {
