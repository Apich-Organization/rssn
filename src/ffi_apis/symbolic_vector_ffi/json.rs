...
#[no_mangle]
pub extern "C" fn rssn_json_vector_dot(v1_json: *const c_char, v2_json: *const c_char) -> *mut c_char {
...
#[no_mangle]
pub extern "C" fn rssn_json_vector_cross(v1_json: *const c_char, v2_json: *const c_char) -> *mut c_char {
    let v1: Option<Vector> = from_json_string(v1_json);
    let v2: Option<Vector> = from_json_string(v2_json);
...
pub extern "C" fn rssn_json_vector_gradient(
    ...
    let x_str = unsafe {
        if x_var.is_null() { None } else { std::ffi::CStr::from_ptr(x_var).to_str().ok() }
    };
    let y_str = unsafe {
        if y_var.is_null() { None } else { std::ffi::CStr::from_ptr(y_var).to_str().ok() }
    };
    let z_str = unsafe {
        if z_var.is_null() { None } else { std::ffi::CStr::from_ptr(z_var).to_str().ok() }
    };
