...
#[no_mangle]
pub extern "C" fn rssn_bincode_differentiate(expr_buf: BincodeBuffer, var: *const c_char) -> BincodeBuffer {
...
#[no_mangle]
pub extern "C" fn rssn_bincode_integrate(expr_buf: BincodeBuffer, var: *const c_char) -> BincodeBuffer {
    let expr: Option<Expr> = from_bincode_buffer(&expr_buf);
    let var_str = unsafe {
        if var.is_null() { None } else { std::ffi::CStr::from_ptr(var).to_str().ok() }
    };
...
#[no_mangle]
pub extern "C" fn rssn_bincode_limit(expr_buf: BincodeBuffer, var: *const c_char, point_buf: BincodeBuffer) -> BincodeBuffer {
    let expr: Option<Expr> = from_bincode_buffer(&expr_buf);
...
#[no_mangle]
pub extern "C" fn rssn_bincode_find_poles(expr_buf: BincodeBuffer, var: *const c_char) -> BincodeBuffer {
    let expr: Option<Expr> = from_bincode_buffer(&expr_buf);
    let var_str = unsafe {
        if var.is_null() { None } else { std::ffi::CStr::from_ptr(var).to_str().ok() }
    };
...
#[no_mangle]
pub extern "C" fn rssn_bincode_calculate_residue(expr_buf: BincodeBuffer, var: *const c_char, pole_buf: BincodeBuffer) -> BincodeBuffer {
    let expr: Option<Expr> = from_bincode_buffer(&expr_buf);
...
#[no_mangle]
pub extern "C" fn rssn_bincode_find_pole_order(expr_buf: BincodeBuffer, var: *const c_char, pole_buf: BincodeBuffer) -> usize {
    let expr: Option<Expr> = from_bincode_buffer(&expr_buf);
    let pole: Option<Expr> = from_bincode_buffer(&pole_buf);
    let var_str = unsafe {
        if var.is_null() { None } else { std::ffi::CStr::from_ptr(var).to_str().ok() }
...
#[no_mangle]
pub extern "C" fn rssn_bincode_substitute(expr_buf: BincodeBuffer, var: *const c_char, replacement_buf: BincodeBuffer) -> BincodeBuffer {
    let expr: Option<Expr> = from_bincode_buffer(&expr_buf);
...
#[no_mangle]
pub extern "C" fn rssn_bincode_path_integrate(expr_buf: BincodeBuffer, var: *const c_char, contour_buf: BincodeBuffer) -> BincodeBuffer {
    let expr: Option<Expr> = from_bincode_buffer(&expr_buf);
...
#[no_mangle]
pub extern "C" fn rssn_bincode_evaluate_at_point(expr_buf: BincodeBuffer, var: *const c_char, value_buf: BincodeBuffer) -> BincodeBuffer {
    let expr: Option<Expr> = from_bincode_buffer(&expr_buf);
