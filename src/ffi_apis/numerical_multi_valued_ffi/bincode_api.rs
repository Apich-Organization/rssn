...
#[no_mangle]
pub unsafe extern "C" fn rssn_num_mv_newton_method_complex_bincode(buffer: BincodeBuffer) -> BincodeBuffer {
    let input: NewtonInput = match from_bincode_buffer(&buffer) {
        Some(i) => i,
        None => return to_bincode_buffer(&FfiResult::<ComplexResult, String> { ok: None, err: Some("Invalid Bincode input".to_string()) }),
    };
    ...
    match multi_valued::newton_method_complex(&input.f, &input.f_prime, start_point, input.tolerance, input.max_iter) {
        ...
        None => to_bincode_buffer(&FfiResult::<ComplexResult, String> { ok: None, err: Some("Newton's method failed to converge".to_string()) }),
    }
}
...
pub unsafe extern "C" fn rssn_num_mv_complex_log_k_bincode(buffer: BincodeBuffer) -> BincodeBuffer {
    ...
    to_bincode_buffer(&FfiResult { ok: Some(out), err: None::<String> })
}

#[no_mangle]
pub unsafe extern "C" fn rssn_num_mv_complex_sqrt_k_bincode(buffer: BincodeBuffer) -> BincodeBuffer {
    let input: LogSqrtInput = match from_bincode_buffer(&buffer) {
        Some(i) => i,
        None => return to_bincode_buffer(&FfiResult::<ComplexResult, String> { ok: None, err: Some("Invalid Bincode input".to_string()) }),
    };
