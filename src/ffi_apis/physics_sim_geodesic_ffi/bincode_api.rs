...
#[no_mangle]
pub unsafe extern "C" fn rssn_physics_sim_geodesic_run_bincode(buffer: BincodeBuffer) -> BincodeBuffer {
    let params: GeodesicParameters = match from_bincode_buffer(&buffer) {
        Some(p) => p,
        None => return to_bincode_buffer(&FfiResult::<Vec<(f64, f64)>, String>::err("Invalid Bincode".to_string())),
    };
