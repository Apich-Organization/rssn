...
pub unsafe extern "C" fn rssn_num_sparse_create(
    ...
    let r_idx = unsafe { std::slice::from_raw_parts(row_indices, nnz) };
    let c_idx = unsafe { std::slice::from_raw_parts(col_indices, nnz) };
    let vals = unsafe { std::slice::from_raw_parts(values, nnz) };
...
pub unsafe extern "C" fn rssn_num_sparse_spmv(
    ...
    let m = unsafe { &*matrix };
    let v = unsafe { std::slice::from_raw_parts(vector, vec_len) };
