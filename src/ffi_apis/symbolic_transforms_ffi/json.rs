...
pub unsafe extern "C" fn rssn_json_fourier_scaling(
    ...
    match (f, a, out_var) {
        | (Some(f), Some(a), Some(ov)) => to_json_string(&transforms::fourier_scaling(&f, &a, &ov)),
        | _ => std::ptr::null_mut(),
    }
}
...
pub unsafe extern "C" fn rssn_json_fourier_differentiation(
    ...
    match (f, out_var) {
        | (Some(f), Some(ov)) => to_json_string(&transforms::fourier_differentiation(&f, &ov)),
        | _ => std::ptr::null_mut(),
    }
}
...
pub unsafe extern "C" fn rssn_json_laplace_scaling(
    ...
    match (f, a, out_var) {
        | (Some(f), Some(a), Some(ov)) => to_json_string(&transforms::laplace_scaling(&f, &a, &ov)),
        | _ => std::ptr::null_mut(),
    }
}
...
pub unsafe extern "C" fn rssn_json_laplace_integration(
    ...
    match (f, out_var) {
        | (Some(f), Some(ov)) => to_json_string(&transforms::laplace_integration(&f, &ov)),
        | _ => std::ptr::null_mut(),
    }
}
...
pub unsafe extern "C" fn rssn_json_z_transform(
    ...
    match (expr, in_var, out_var) {
        | (Some(e), Some(iv), Some(ov)) => to_json_string(&transforms::z_transform(&e, &iv, &ov)),
        | _ => std::ptr::null_mut(),
    }
}
...
pub unsafe extern "C" fn rssn_json_z_time_shift(
    ...
    match (f, k, out_var) {
        | (Some(f), Some(k), Some(ov)) => to_json_string(&transforms::z_time_shift(&f, &k, &ov)),
        | _ => std::ptr::null_mut(),
    }
}
...
pub unsafe extern "C" fn rssn_json_z_scaling(
    ...
    match (f, a, out_var) {
        | (Some(f), Some(a), Some(ov)) => to_json_string(&transforms::z_scaling(&f, &a, &ov)),
        | _ => std::ptr::null_mut(),
    }
}
...
pub unsafe extern "C" fn rssn_json_z_differentiation(
    ...
    match (f, out_var) {
        | (Some(f), Some(ov)) => to_json_string(&transforms::z_differentiation(&f, &ov)),
        | _ => std::ptr::null_mut(),
    }
}
