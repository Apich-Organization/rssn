...
use crate::jit::{JitEngine, Instruction};
...
pub unsafe extern "C" fn rssn_jit_compile_json(
    ...
    let req: JitCompileRequest = match serde_json::from_str(json_str) {
        Ok(r) => r,
        Err(e) => {
            let res: FfiResult<usize, String> = FfiResult { ok: None, err: Some(e.to_string()) };
    ...
    match engine.compile(&req.instructions) {
        Ok(ptr) => {
            let res: FfiResult<usize, String> = FfiResult { ok: Some(ptr as usize), err: None };
            CString::new(serde_json::to_string(&res).unwrap()).unwrap().into_raw()
        },
        Err(e) => {
            let res: FfiResult<usize, String> = FfiResult { ok: None, err: Some(e) };
            CString::new(serde_json::to_string(&res).unwrap()).unwrap().into_raw()
        }
    }
}
