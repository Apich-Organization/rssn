...
fn test_air_kinematic_viscosity_handle() {
    let nu = rssn::ffi_apis::numerical_physics_cfd_ffi::handle::rssn_num_cfd_air_kinematic_viscosity();
...
fn test_water_kinematic_viscosity_handle() {
    let nu = rssn::ffi_apis::numerical_physics_cfd_ffi::handle::rssn_num_cfd_water_kinematic_viscosity();
...
fn test_mach_number_handle() {
    let ma = rssn::ffi_apis::numerical_physics_cfd_ffi::handle::rssn_num_cfd_mach_number(250.0, 340.0);
    assert!((ma - 0.735).abs() < 0.01);
}

#[test]
fn test_froude_number_handle() {
    let fr = rssn::ffi_apis::numerical_physics_cfd_ffi::handle::rssn_num_cfd_froude_number(5.0, 10.0, 9.81);
    assert!(fr > 0.4 && fr < 0.6);
}

#[test]
fn test_cfl_number_handle() {
    let cfl = rssn::ffi_apis::numerical_physics_cfd_ffi::handle::rssn_num_cfd_cfl_number(1.0, 0.01, 0.1);
    assert!((cfl - 0.1).abs() < 1e-10);
}

#[test]
fn test_check_cfl_stability_handle() {
    let stable = rssn::ffi_apis::numerical_physics_cfd_ffi::handle::rssn_num_cfd_check_cfl_stability(
        1.0, 0.01, 0.1, 1.0,
    );
    assert!(stable);
    
    let unstable = rssn::ffi_apis::numerical_physics_cfd_ffi::handle::rssn_num_cfd_check_cfl_stability(
        10.0, 0.1, 0.1, 1.0,
    );
    assert!(!unstable);
}
...
fn test_diffusion_number_handle() {
    let r = rssn::ffi_apis::numerical_physics_cfd_ffi::handle::rssn_num_cfd_diffusion_number(0.01, 0.001, 0.01);
...
fn test_air_properties_json() {
    ...
    unsafe {
        let result = rssn::ffi_apis::numerical_physics_cfd_ffi::json::rssn_num_cfd_air_properties_json(c_input.as_ptr());
...
fn test_water_properties_json() {
    ...
    unsafe {
        let result = rssn::ffi_apis::numerical_physics_cfd_ffi::json::rssn_num_cfd_water_properties_json(c_input.as_ptr());
        assert!(!result.is_null());
        let result_str = std::ffi::CStr::from_ptr(result).to_string_lossy();
        let parsed: serde_json::Value = serde_json::from_str(&result_str).unwrap();
        let pr = parsed["ok"]["prandtl_number"].as_f64().unwrap();
...
fn test_reynolds_number_json() {
    ...
    unsafe {
        let result = rssn::ffi_apis::numerical_physics_cfd_ffi::json::rssn_num_cfd_reynolds_number_json(c_input.as_ptr());
        assert!(!result.is_null());
        let result_str = std::ffi::CStr::from_ptr(result).to_string_lossy();
        let parsed: serde_json::Value = serde_json::from_str(&result_str).unwrap();
...
fn test_cfl_number_json() {
    ...
    unsafe {
        let result = rssn::ffi_apis::numerical_physics_cfd_ffi::json::rssn_num_cfd_cfl_number_json(c_input.as_ptr());
        assert!(!result.is_null());
        let result_str = std::ffi::CStr::from_ptr(result).to_string_lossy();
        let parsed: serde_json::Value = serde_json::from_str(&result_str).unwrap();
...
fn test_solve_advection_1d_json() {
    ...
    unsafe {
        let result = rssn::ffi_apis::numerical_physics_cfd_ffi::json::rssn_num_cfd_solve_advection_1d_json(c_input.as_ptr());
        assert!(!result.is_null());
        let result_str = std::ffi::CStr::from_ptr(result).to_string_lossy();
        let parsed: serde_json::Value = serde_json::from_str(&result_str).unwrap();
...
fn test_solve_diffusion_1d_json() {
    ...
    unsafe {
        let result = rssn::ffi_apis::numerical_physics_cfd_ffi::json::rssn_num_cfd_solve_diffusion_1d_json(c_input.as_ptr());
        assert!(!result.is_null());
        let result_str = std::ffi::CStr::from_ptr(result).to_string_lossy();
        let parsed: serde_json::Value = serde_json::from_str(&result_str).unwrap();
        let data = parsed["ok"].as_array().unwrap();
...
fn test_solve_burgers_1d_json() {
    ...
    unsafe {
        let result = rssn::ffi_apis::numerical_physics_cfd_ffi::json::rssn_num_cfd_solve_burgers_1d_json(c_input.as_ptr());
        assert!(!result.is_null());
        let result_str = std::ffi::CStr::from_ptr(result).to_string_lossy();
        let parsed: serde_json::Value = serde_json::from_str(&result_str).unwrap();
        let data = parsed["ok"].as_array().unwrap();
