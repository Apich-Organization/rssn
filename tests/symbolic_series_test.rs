...
fn test_taylor_series_exp() {
    ...
    let val_at_0_5 = rssn::symbolic::calculus::substitute(&series, "x", &Expr::new_constant(0.5));

    assert_is_value(&val_at_0_5, 1.645833);
}
...
fn test_taylor_series_sin() {
    ...
    let val_at_0_1 = rssn::symbolic::calculus::substitute(&series, "x", &Expr::new_constant(0.1));

    assert_is_value(&val_at_0_1, 0.0998333);
}
...
fn test_asymptotic_expansion() {
    ...
    let series = asymptotic_expansion(&expr, "x", &Expr::Infinity, 2);

    // Evaluate at x = 100
    // Exact: 101/99 = 1.020202...
    // Series: 1 + 2/100 + 2/10000 = 1.0202

    let val_at_100 = rssn::symbolic::calculus::substitute(&series, "x", &Expr::new_constant(100.0));
