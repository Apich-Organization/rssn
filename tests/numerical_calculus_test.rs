...
fn test_gradient() {
    ...
    assert_approx_eq!(grad[1], 2.0, 1e-5f64);
}
...
fn test_jacobian() {
    ...
    assert_approx_eq!(jac[0][1], 1.0, 1e-5f64);
    assert_approx_eq!(jac[1][0], 2.0, 1e-5f64);
    assert_approx_eq!(jac[1][1], 4.0, 1e-5f64);
}
...
fn test_hessian() {
    ...
    // H at (1,2) = [[4, 2], [2, 12]]
    assert_approx_eq!(hess[0][0], 4.0, 1e-4f64);
    assert_approx_eq!(hess[0][1], 2.0, 1e-4f64);
    assert_approx_eq!(hess[1][0], 2.0, 1e-4f64);
    assert_approx_eq!(hess[1][1], 12.0, 1e-4f64);
}
