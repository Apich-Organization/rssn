...
fn test_method_calls() {
    ...
    // Exp/Log
    assert_eq!(format!("{:?}", x.exp()), "exp(x)");
    assert_eq!(format!("{:?}", x.ln()), "ln(x)");

    // Pow/Sqrt
    assert_eq!(format!("{:?}", x.pow(Expr::new_constant(2.0))), "(x^(2))");
    assert_eq!(format!("{:?}", x.sqrt()), "sqrt(x)");

    // Abs
    assert_eq!(format!("{:?}", x.abs()), "|x|");
...
fn test_complex_expressions() {
    ...
    assert_eq!(format!("{:?}", expr2), "((sin(x)^(2)) + (cos(x)^(2)))");
}
