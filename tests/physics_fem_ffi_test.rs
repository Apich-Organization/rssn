...
fn test_fem_1d_json_ffi() {
    ...
    unsafe {
        let res_ptr = rssn::ffi_apis::physics_fem_ffi::json::rssn_physics_fem_solve_poisson_1d_json(c_input.as_ptr());
...
fn test_fem_1d_handle_ffi() {
    let mut out_size = 0;
    unsafe {
        let res_ptr = rssn::ffi_apis::physics_fem_ffi::handle::rssn_physics_fem_solve_poisson_1d(10, 1.0, &mut out_size);
        assert!(!res_ptr.is_null());
