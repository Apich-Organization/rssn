...
fn test_evaluate_action_harmonic_oscillator() {
    ...
    let action = evaluate_action(&lagrangian, &path, "t", "y", "y_dot", (0.0, std::f64::consts::PI)).unwrap();
...
fn test_euler_lagrange_free_particle() {
    // L = 0.5 * y_dot^2
    let lagrangian = Expr::new_mul(Expr::new_constant(0.5), Expr::new_pow(Expr::new_variable("y_dot"), Expr::new_constant(2.0)));
