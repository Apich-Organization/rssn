...
fn test_gamma_numerical() {
    ...
    // Gamma(1) = 1
    assert_approx_eq(gamma_numerical(1.0), 1.0);
}
...
fn test_ln_gamma_numerical() {
    ...
    // ln(Gamma(1)) = 0
    assert_approx_eq(ln_gamma_numerical(1.0), 0.0);
}
...
fn test_digamma_numerical() {
    ...
    assert_approx_eq(digamma_numerical(1.0), -euler_mascheroni);

    // ψ(2) = 1 - γ
    assert_approx_eq(digamma_numerical(2.0), 1.0 - euler_mascheroni);
}
...
fn test_beta_numerical() {
    ...
    assert_approx_eq(beta_numerical(3.0, 5.0), beta_numerical(5.0, 3.0));
}
...
fn test_ln_beta_numerical() {
    // ln(B(2, 2)) = ln(1/6)
    assert_approx_eq(ln_beta_numerical(2.0, 2.0), (1.0 / 6.0_f64).ln());
}
...
fn test_regularized_incomplete_beta() {
    // I_0(a, b) = 0
    assert_approx_eq(regularized_incomplete_beta(2.0, 3.0, 0.0), 0.0);

    // I_1(a, b) = 1
    assert_approx_eq(regularized_incomplete_beta(2.0, 3.0, 1.0), 1.0);

    // I_0.5(1, 1) = 0.5 (uniform distribution)
    assert_approx_eq(regularized_incomplete_beta(1.0, 1.0, 0.5), 0.5);
}
...
fn test_erf_numerical() {
    ...
    assert_approx_eq(erf_numerical(-1.0), -erf_numerical(1.0));
}
...
fn test_binomial() {
    ...
    assert_eq!(binomial(15, 6), binomial(15, 9));
...
fn test_rising_factorial() {
    // (1)_n = n!
    assert_approx_eq(rising_factorial(1.0, 5), 120.0);

    // (3)_4 = 3 * 4 * 5 * 6 = 360
    assert_approx_eq(rising_factorial(3.0, 4), 360.0);

    // (x)_0 = 1
    assert_approx_eq(rising_factorial(5.0, 0), 1.0);
}
...
fn test_falling_factorial() {
    // (5)_{(3)} = 5 * 4 * 3 = 60
    assert_approx_eq(falling_factorial(5.0, 3), 60.0);

    // (10)_{(4)} = 10 * 9 * 8 * 7 = 5040
    assert_approx_eq(falling_factorial(10.0, 4), 5040.0);

    // (x)_{(0)} = 1
    assert_approx_eq(falling_factorial(5.0, 0), 1.0);

    // (n)_{(n)} = n!
    assert_approx_eq(falling_factorial(5.0, 5), 120.0);
}
...
fn test_ln_factorial() {
    ...
    assert_approx_eq(ln_factorial(5), 120.0_f64.ln());

    assert_approx_eq(ln_factorial(10), 3628800.0_f64.ln());
}
...
fn test_bessel_j0() {
    ...
    println!("bessel_j0(2.405) = {}", bessel_j0(2.405));
...
fn test_bessel_y0() {
    ...
    println!("bessel_y0(0.1) = {}", bessel_y0(0.1));
...
fn test_bessel_i1() {
    ...
    assert_approx_eq(bessel_i1(-1.0), -bessel_i1(1.0));
}
...
fn test_sinc() {
    ...
    assert_approx_eq(sinc(0.5), 2.0 / std::f64::consts::PI);
}
...
fn test_regularized_gamma_p() {
    ...
    assert_approx_eq(regularized_gamma_p(1.0, x), 1.0 - (-x).exp());
...
fn test_regularized_gamma_q() {
    ...
    assert_approx_eq(regularized_gamma_p(a, x) + regularized_gamma_q(a, x), 1.0);

    // Q(1, x) = e^(-x)
    assert_approx_eq(regularized_gamma_q(1.0, x), (-x).exp());
}
...
fn test_gamma_beta_relationship() {
    ...
    assert_approx_eq(beta_direct, beta_from_gamma);
}
...
fn test_binomial_factorial_relationship() {
    ...
    assert_eq!(binomial_val, factorial_val);
}
...
fn test_bessel_recurrence() {
    ...
    println!("j2_from_recurrence = {}", j2_from_recurrence);
