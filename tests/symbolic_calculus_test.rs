...
fn test_limit() {
    ...
    match &lim_x {
        ...
        | Expr::Dag(_) => {
            println!("DEBUG: lim_x is Dag")
        },
        | _ => {
            println!("DEBUG: lim_x is {:?}", lim_x)
        },
    }

    assert_eq!(lim_x, Expr::new_constant(0.0));
...
fn test_poles_and_residues() {
    ...
    match &res {
        ...
        | Expr::Dag(_) => {
            println!("DEBUG: res is Dag")
        },
        | _ => {
            println!("DEBUG: res is {:?}", res)
        },
    }

    assert_eq!(res, Expr::new_constant(1.0));
}
