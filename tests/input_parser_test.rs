...
fn test_parse_bigint() {
    ...
    // In your test
    let left = parse_expr("123").unwrap().1;

    let right = Expr::BigInt(BigInt::from(123));
    ...
    assert_eq!(parse_expr("-456").expect("Parse Expr failed.").1, expected);
}
...
fn test_parse_volume_integral() {
    ...
    print_type_of(&parse_expr("volume_integral(f(x,y,z), V)"));
