...
fn test_numerical_calculus_handle_ffi() {
    unsafe {
        ...
        let status = handle::rssn_num_calculus_partial_derivative(&f, var_name.as_ptr(), 2.0, &mut res);
...
fn test_numerical_gradient_json_ffi() {
    unsafe {
        ...
        let v: serde_json::Value = serde_json::from_str(res_str).expect("Failed to parse result JSON");
...
fn test_numerical_hessian_bincode_ffi() {
    unsafe {
        ...
        let res: FfiResult<Vec<Vec<f64>>, String> = from_bincode_buffer(&res_buffer).expect("Failed to decode bincode result");
