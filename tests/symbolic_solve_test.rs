...
fn test_solve_cubic() {
    ...
    let mut vals: Vec<f64> = solutions
        .iter()
        .map(|s| evaluate_numerical(s).expect("Failed to evaluate"))
        .collect();
    ...
    assert!((vals[1] - 2.0).abs() < 1e-7);

    assert!((vals[2] - 3.0).abs() < 1e-7);
}
...
fn test_solve_quartic_biquadratic() {
    ...
    let eq = Expr::new_add(
        Expr::new_add(
            Expr::new_pow(x.clone(), Expr::new_constant(4.0)),
            Expr::new_mul(
    ...
    assert!((vals[0] - -2.0).abs() < 1e-7);

    assert!((vals[1] - -1.0).abs() < 1e-7);

    assert!((vals[2] - 1.0).abs() < 1e-7);

    assert!((vals[3] - 2.0).abs() < 1e-7);
}
...
fn test_solve_quartic_general() {
    ...
    let eq = Expr::new_add(
        Expr::new_add(
            ...
            Expr::new_mul(Expr::new_constant(-50.0), x),
        ),
