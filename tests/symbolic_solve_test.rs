...
fn assert_is_value(expr: &Expr, expected: f64) {
    let val = evaluate_numerical(expr).unwrap_or_else(|| panic!("Failed to evaluate expression: {:?}", expr));
...
fn test_solve_quadratic() {
    ...
    let has_2 = solutions.iter().any(|s| evaluate_numerical(s).map(|v| (v - 2.0).abs() < 1e-10).unwrap_or(false));
    let has_neg_2 = solutions.iter().any(|s| evaluate_numerical(s).map(|v| (v - -2.0).abs() < 1e-10).unwrap_or(false));
...
fn test_solve_system_substitution() {
    ...
    let x_sol = solutions.iter().find(|(v, _)| matches!(v, Expr::Variable(name) if name == "x"));
    let y_sol = solutions.iter().find(|(v, _)| matches!(v, Expr::Variable(name) if name == "y"));
