...
fn test_numerical_sum_series_json_ffi() {
    unsafe {
        ...
        assert_approx_eq!(v["ok"].as_f64().unwrap(), 55.0, 1e-10f64);
...
fn test_numerical_taylor_bincode_ffi() {
    unsafe {
        ...
        let res: FfiResult<Vec<f64>, String> = from_bincode_buffer(&res_buffer).expect("Failed to decode bincode result");
