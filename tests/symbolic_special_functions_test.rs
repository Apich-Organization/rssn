...
fn test_chebyshev_t_basic() {
    ...
    // T_0(x) = 1
    let t0 = chebyshev_t(&Expr::Constant(0.0), &x);

    assert_approx_eq(&t0, 1.0);

    // T_1(x) = x
    let t1 = chebyshev_t(&Expr::Constant(1.0), &x);
...
fn test_chebyshev_u_basic() {
    ...
    // U_0(x) = 1
    let u0 = chebyshev_u(&Expr::Constant(0.0), &x);
...
fn test_differential_equations_construct() {
    ...
    match bessel_eq {
        ...
        | _ => {
            panic!("Expected Eq, got {:?}", bessel_eq)
        },
    }
    ...
    match legendre_eq {
        ...
        | _ => {
            panic!("Expected Eq, got {:?}", legendre_eq)
        },
    }
    ...
    match laguerre_eq {
        ...
        | _ => {
            panic!("Expected Eq, got {:?}", laguerre_eq)
        },
    }
    ...
    match hermite_eq {
        ...
        | _ => {
            panic!("Expected Eq, got {:?}", hermite_eq)
        },
    }
    ...
    match chebyshev_eq {
        ...
        | _ => {
            panic!("Expected Eq, got {:?}", chebyshev_eq)
        },
    }
}
...
fn test_rodrigues_formulas_construct() {
    ...
    match legendre_rf {
        ...
        | _ => {
            panic!("Expected Eq, got {:?}", legendre_rf)
        },
    }
    ...
    match hermite_rf {
        ...
        | _ => {
            panic!("Expected Eq, got {:?}", hermite_rf)
        },
    }
}
