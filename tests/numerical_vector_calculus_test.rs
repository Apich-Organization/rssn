...
fn test_curl_expr() {
    ...
    let f2 = Expr::new_mul(Expr::new_constant(-1.0), x.clone());

    let f3 = Expr::new_constant(0.0);

    let res = curl_expr(&[f1, f2, f3], &["x", "y", "z"], &[1.0, 1.0, 1.0]).unwrap();

    assert_approx_eq!(res[0], 0.0, 1e-5);

    assert_approx_eq!(res[1], 0.0, 1e-5);

    assert_approx_eq!(res[2], -2.0, 1e-5);
}
