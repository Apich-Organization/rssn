...
fn test_metric_tensor_symbolic() {
    ...
    let inv_00 = metric.g_inv.get(&[0, 0]).unwrap().to_ast().unwrap_or(metric.g_inv.get(&[0, 0]).unwrap().clone());
    let inv_11 = metric.g_inv.get(&[1, 1]).unwrap().to_ast().unwrap_or(metric.g_inv.get(&[1, 1]).unwrap().clone());
