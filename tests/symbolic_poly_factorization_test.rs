...
use num_traits::One;
use num_traits::Zero;
use rssn::symbolic::finite_field::FiniteFieldPolynomial;
...
fn test_poly_derivative_gf() {
    ...
    assert_eq!(
        derivative.coeffs[0].value,
        BigInt::from(2)
    ); // coefficient of x
    assert_eq!(
        derivative.coeffs[1].value,
        BigInt::from(2)
    ); // constant term
}
...
fn test_poly_gcd_gf() {
    ...
    let poly1 = create_poly(
        vec![1, 0, -1],
        field.clone(),
    ); // x^2 - 1
    let poly2 = create_poly(
        vec![1, -1],
        field.clone(),
    ); // x - 1
...
fn test_poly_pow_mod() {
    ...
    let base = create_poly(
        vec![1, 0],
        field.clone(),
    ); // x
    let modulus = create_poly(
        vec![1, 0, 1],
        field.clone(),
    ); // x^2 + 1
