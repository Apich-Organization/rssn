...
fn test_finite_field_polynomial_degree() {
    ...
    // Zero polynomial
    let zero_poly = FiniteFieldPolynomial::new(&[], field.clone());
...
fn test_extension_field_element_creation() {
    ...
    let irreducible = FiniteFieldPolynomial::new(
        &[
            PrimeFieldElement::new(BigInt::from(1), prime_field.clone()),
            PrimeFieldElement::new(BigInt::from(0), prime_field.clone()),
...
fn test_extension_field_element_arithmetic() {
    ...
    let poly2 = FiniteFieldPolynomial::new(
        &[
            PrimeFieldElement::new(BigInt::from(2), prime_field.clone()),
            PrimeFieldElement::new(BigInt::from(3), prime_field.clone()),
        ],
        prime_field.clone(),
    );
    ...
    // Test multiplication
    let product = elem1.mul(elem2).expect(
