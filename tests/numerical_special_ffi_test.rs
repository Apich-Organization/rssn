use rssn::ffi_apis::numerical_special_ffi::{handle, json, bincode_api};
use rssn::ffi_apis::common::{rssn_free_string, rssn_free_bincode_buffer, to_bincode_buffer, from_bincode_buffer};
...
fn test_special_json_ffi() {
    unsafe {
        ...
        // Binomial
        let input = BinomialInput { n: 5, k: 2 };
        let json_str = serde_json::to_string(&input).unwrap();
        let c_json = CString::new(json_str).unwrap();
...
fn test_special_bincode_ffi() {
    unsafe {
        #[derive(Serialize)]
        struct SingleInputB { x: f64 }
        
        // Gamma
        let input = SingleInputB { x: 5.0 };
