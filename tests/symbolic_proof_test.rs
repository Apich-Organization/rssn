...
fn test_verify_equation_solution() {
    ...
    let eq = Expr::new_sub(Expr::new_pow(Expr::new_variable("x"), Expr::Constant(2.0)), Expr::Constant(4.0));
...
fn test_verify_derivative() {
    ...
    let df = Expr::new_mul(Expr::Constant(3.0), Expr::new_pow(Expr::new_variable("x"), Expr::Constant(2.0)));
...
fn test_verify_limit() {
    // lim_{x->0} sin(x)/x = 1
    let f = Expr::new_div(Expr::new_sin(Expr::new_variable("x")), Expr::new_variable("x"));
