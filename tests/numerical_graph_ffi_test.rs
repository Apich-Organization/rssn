use rssn::ffi_apis::numerical_graph_ffi::{handle, json, bincode_api};
...
fn test_graph_bincode_ffi() {
    unsafe {
        ...
        // Connected Components
        #[derive(Serialize)]
        struct GraphDefIn { // Need separate struct if reusing fields not optimal, but GraphDef is fine
             num_nodes: usize,
             edges: Vec<Edge>,
        }
