...
fn test_solve_ode_rk4_exponential() {
    ...
    let res = solve_ode_system_rk4(&[y0], &y_init, (0.0, 1.0), 100).unwrap();

    // y(1) = e^1 approx 2.71828
    assert_approx_eq!(res.last().unwrap()[0], 2.71828, 1e-5);
}
...
fn test_solve_ode_euler_exponential() {
    ...
    let res = solve_ode_euler(&[y0], &y_init, (0.0, 1.0), 1000).unwrap();

    // Euler is less accurate, needs more steps
    assert_approx_eq!(res.last().unwrap()[0], 2.71828, 1e-2);
}
...
fn test_solve_ode_heun_exponential() {
    ...
    let res = solve_ode_heun(&[y0], &y_init, (0.0, 1.0), 100).unwrap();

    assert_approx_eq!(res.last().unwrap()[0], 2.71828, 1e-3);
}
